...
durable streams. Carries `weft.events.>` traffic produced by every
weft mutation and consumed by `weft events`, `weft-microvm events`, and
...
```text
weft.events.<kind>                                 # global (operator-visible)
weft.events.project.<uuid>.events.<kind>           # per-project mirror (tenant-visible, read-only)
weft.events.project.<uuid>.app.<kind>              # per-project app stream (tenant publish + subscribe)
```

Tenant-safe kinds (`vm.*`, `guest.*`, `server.*`, `volume.*`,
`network.*`) are dual-published by weft onto both the global and
...
`NATSEventBus.Publish` writes to `weft.events.<kind>`. No tenant
access yet ; weft / weft-microvm read with an OIDC token via weft's
`WatchEvents` RPC.
...
virtio-fs share with tag `weft-nats`. Tenants authenticate with
...
authorization {
  ...
  users = [
    { nkey: "U…<weft-admin>", permissions: {
      publish:   { allow: ["weft.>"] }
      subscribe: { allow: ["weft.>"] }
    } },
    # one entry per project with a NATSUserSeed
    { nkey: "U…<project-1-pubkey>", permissions: {
      subscribe: { allow: ["weft.events.project.<uuid-1>.events.>"] }
      publish:   { allow: ["weft.events.project.<uuid-1>.app.>"] }
    } },
    …
  ]
}
...
Enable it from `weft.hcl` with a `nats_authorization { ... }`
block :
...
- Future : weft emits a `nats.authorization_changed` control-plane
  event ; a small in-VM sidecar watches the bus and signals
  nats-server.
- Future : `weft admin nats reload-authz` RPC pushes the render
  through weft to each NATS micro-VM and triggers `signal reload`
  remotely.
...
entry on `weft.events.project.<uuid>.app.>` for every project — the
...
- Wire weft to auto-render the block on every project mutation and
  push it to each NATS micro-VM, then signal `nats-server --signal
  reload` remotely (today the operator runs `weft admin nats-authz`
