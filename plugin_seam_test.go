...
// plugin_seam_test.go installs an in-process driver set for the local host so
// the weft package's tests never spawn the external weft-driver-* plugin
// executables (weft.New → initLocalDrivers → localDriverBundle would otherwise
// try to launch one). The real go-plugin launcher is exercised end-to-end by
// the handshake smoke in the weft-driver-plugin module instead.
