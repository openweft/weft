...
func main() {
	...
	root := &cobra.Command{
		Use:   "weft-e2e",
		Short: "End-to-end test harness for a live weft deployment",
		Args:  cobra.NoArgs,
		// A failed assertion is a result, not a usage mistake, and the
		// runner has already printed it. Cobra prints neither the usage
		// block nor the error a second time.
		SilenceUsage:  true,
		SilenceErrors: true,
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			// The runner's code is the process's code : 0 on full success,
			// non-zero on any assertion failure. Returning it as an error
			// would flatten every failure count to 2.
